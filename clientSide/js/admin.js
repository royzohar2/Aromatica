$(document).ready(function () {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        data.forEach(function (perfume) {
          ...
          content +=
            "<td><button class='btn btn-danger delete-button' data-perfume-id='" +
            perfume._id +
            "'>Delete</button>";
          content +=
            "<button class='btn btn-primary edit-button ml-2' data-perfume-id='" +
            perfume._id +
            "'>Edit</button></td>";
        ...
        $(".admin-content").append(
          "<button id='newperfume' class='btn btn-primary edit-button ml-2'>New Perfume</button>"
        );
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        data.forEach(function (client) {
          ...
          content +=
            "<td><button class='btn btn-danger delete-button' data-client-id='" +
            client._id +
            "'>Delete</button>";
          content +=
            "<button class='btn btn-primary edit-button ml-2' data-client-id='" +
            client._id +
            "'>Edit</button></td>";
  ...
  $("#order").click(async function () {
    $(".admin-content").html("<h3>Loading Data!!</h3>");
    try {
      ...
      let content = "<h3>Orders Content</h3>";
      content += "<table class='table table-bordered'>";
      content += "<thead>";
      content += "<tr>";
      content += "<th>User</th>";
      content += "<th>Total Amount</th>";
      content += "<th>Product Names</th>";
      ...
      const processedOrders = await Promise.all(orderData.map(processOrder));
      content += processedOrders.join("");
  ...
  async function processOrder(order) {
    ...
    const productDataArray = productResults
      .filter((result) => result.status === "fulfilled")
      .map((result) => result.value);

    const productNames = productDataArray
      .map((productData) => `${productData.name} - (${productData.brand})\n`)
      .join(",");

    let content = "<tr>";
    content += `<td>${userName}</td>`;
    content += `<td>${order.totalAmount}</td>`;
    content += `<td>${productNames}</td>`;
  ...
  function populateEditOrderModal(orderData) {
    ...
    modal
      .find(".order-details")
      .html(
        `<h5>Order id : ${orderData._id}</h5><h5>choose product to remove</h5>`
      );
    ...
    Promise.all(productPromises)
      .then((productDataArray) => {
        ...
        modal.find("#updateOrder").on("click", function () {
          ...
          var removedProducts = modal
            .find('input[name="removedProducts"]:checked')
            .map(function () {
              return $(this).val();
            })
            .get();
          // Call a function to update the order with removed products
          updateOrderWithRemovedProducts(
            orderData._id,
            orderData.products,
            removedProducts
          );
  ...
  function updateOrderWithRemovedProducts(
    orderId,
    allProducts,
    removedProducts
  ) {
    ...
    if (updatedProducts == []) {
      ...
      Promise.all(productPricePromises).then((productDataArray) => {
        var totalPriceSum = 0;
